...
<body>
    <aside class="sidebar" id="sidebar">
        ...
        <nav class="sidebar-nav">
            ...
            <div class="nav-section">
                ...
                <a href="{{ route('admin.rsvp.index') }}" class="nav-link {{ request()->routeIs('admin.rsvp.*') ? 'active' : '' }}">
    ...
    <script>
        document.addEventListener('click', function(e) {
            ...
            if (window.innerWidth <= 768 && sidebar.classList.contains('open') && !sidebar.contains(e.target) && !e.target.classList.contains('mobile-toggle')) {
                sidebar.classList.remove('open');
            }
        });
        function confirmDelete(formId) { if (confirm('Apakah Anda yakin ingin menghapus data ini?')) { document.getElementById(formId).submit(); } }
    </script>
    @stack('scripts')
</body>
</html>
